...
for module in os.listdir(os.path.dirname(__file__) + '/keras'):
    print("Module: " + module + " loading")
    if module == '__init__.py' or module[-3:] != '.py':
        print("Module: " + module + " loading aborted!")
        continue
    try:
        print("Attempting import_module of: " + (__name__ + '.keras.' + module[:-3]))
        lib = importlib.import_module(__name__ + '.keras.' + module[:-3])
        print("import result: "+ str(lib))
        for name, func in list(lib.__dict__.items()):
            ...
            print("trying register for :" + str(name))
            if callable(func) and hasattr(func, 'handles') and func.__module__ == lib.__name__:
                print("func handles in func: " + str(func.handles))
...
def convert_from_yaml_config(yamlConfig):
    ...
    else:  GNU nano 2.3.1                                File: ../hls4ml/converters/__init__.py                                                                       
            raise Exception("TensorFlow not found. Please install TensorFlow.")
    else:
...
def convert_from_keras_model(model, output_dir='my-hls-test', project_name='myproject',                                                                          fpga_part='xcku115-flvb2104-2-i', clock_period=5, hls_config={}):
    config = create_vivado_config(output_dir=output_dir,                                                                                                             project_name=project_name, fpga_part=fpga_part, clock_period=clock_period)
    config['KerasModel'] = model                                                                                                                                                                                                                                                                                              model_config = hls_config.get('Model', None)
    if model_config is not None:
        if not all(k in model_config for k in ('Precision', 'ReuseFactor')):                                                                                             raise Exception('Precision and ReuseFactor must be provided in the hls_config')
    else:                                                                                                                                                            model_config = {}                                                                                                                                            model_config['Precision'] = 'ap_fixed<16,6>'                                                                                                                 model_config['ReuseFactor'] = '1'
    config['HLSConfig']['Model'] = model_config                                                                                                                                                                                                                                                                               if 'LayerName' in hls_config:                                                                                                                                    config['HLSConfig']['LayerName'] = hls_config['LayerName']                                                                                                                                                                                                                                                            if 'LayerType' in hls_config:                                                                                                                                    config['HLSConfig']['LayerType'] = hls_config['LayerType']
